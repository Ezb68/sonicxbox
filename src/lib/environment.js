...
var Environment = {
  ...
  detect: function (config, callback) {
    ...
    if (!network_config) {
      return callback(new TruffleError("Unknown network \"" + config.network + "\". See your sonicxbox configuration file for available networks."));
    }
    ...
    function detectFromAddress(done) {
      ...
      sonicxWrap._getAccounts(function(err, accounts) {
        if (err) return done(err);
        config.networks[config.network].from = accounts[0];
        config.networks[config.network].privateKey = sonicxWrap._privateKeyByAccount[accounts[0]];
        done();
      });
    }
