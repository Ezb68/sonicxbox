...
function Console(tasks, options) {
  ...
  try {
    this.sonicxWrap = SonicXWrap(options.networks[options.network], {
      verify: true,
      log: options.log
    })
  } catch(err) {
  ...
  // this.sonicxWrap.setHttpProvider(options.provider);
...
Console.prototype.start = function(callback) {
  ...
  this.provision(function(err, abstractions) {
    ...
    self.repl.start({
      prompt: "sonicxbox(" + self.options.network + ")> ",
      context: {
        sonicxWrap: self.sonicxWrap,
      },
