...
var SonicXWeb = require("sonicxweb");
...
function TestRunner(options) {
  ...
  this.known_events = {};
  this.sonicxwrap = SonicXWrap();

  global.sonicxWeb = new SonicXWeb(
    this.sonicxwrap.fullNode,
    this.sonicxwrap.solidityNode,
    this.sonicxwrap.eventServer,
    this.sonicxwrap.defaultPrivateKey
  )

  global.waitForTransactionReceipt = waitForTransactionReceipt(sonicxWeb)
      ...
      async.map(files, function(file, finished) {
        ...
        abis.map(function(abi) {
          if (/event/i.test(abi.type)) {
            ...
            self.known_events[self.sonicxwrap.sha3(signature)] = {
              signature: signature,
              abi_entry: abi
            };
          }
