...
function getWrapper(options = {}) {
  ...
  let solcDir = path.join(homedir(), '.sonicxbox', 'solc');
  ...
  if (options.networks) {
    ...
    try {
      ...
      if (supportedVersions.includes(version)) {
        ...
        console.error(`Error:
SonicXBox supports only the following versions:
...
`)
  ...
  if (!fs.existsSync(soljsonPath)) {
    if (process.env.SONICXBOX_NAME) {
      name = process.env.SONICXBOX_NAME
    }
