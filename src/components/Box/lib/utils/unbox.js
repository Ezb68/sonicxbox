  ...
  return Promise.resolve().then(function() {
    ...
    if (contents.length) {
      var err = "Something already exists at the destination. " +
                "`sonicxbox init` and `sonicxbox unbox` must be executed in an empty folder. " +
  ...
  return new Promise(function(accept, reject) {
    ...
    request(options, function(error, r) {
      if (error) {
        ...
        return reject(new Error("sonicxbox Box at URL " + url + " doesn't exist. If you believe this is an error, please contact sonicxbox support."));
...
function readBoxConfig(destination) {
  var possibleConfigs = [
    path.join(destination, "sonicxbox.json"),
    path.join(destination, "sonicxbox-init.json")
  ];
...
function cleanupUnpack(boxConfig, destination) {
  ...
  needingRemoval.push("sonicxbox.json");
  needingRemoval.push("sonicxbox-init.json");
