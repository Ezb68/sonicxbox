...
// The following tests require SonicXBox >= 2.1.x
// and SonicX Quickstart (https://github.com/sonicxprotocol/docker-sonicx-quickstart)
...
contract('MetaCoin', function (accounts) {
  ...
  before(async function () {
    ...
    if(accounts.length < 3) {
      // Set your own accounts if you are not using SonicX Quickstart
  ...
  it("should send coins from account 0 to 3 and verify that a Transfer event has been emitted", function (done) {
    assert.isTrue(accounts[3] ? true : false, 'accounts[1] does not exist. Use SonicX Quickstart!')

    this.timeout(20000)
    MetaCoin.deployed()
        .then(meta => {
          return sonicxWeb.contract().at(meta.address)
              .then(meta2 => {
                meta2.Transfer().watch((err, res) => {
                  if(res) {
                    assert.equal(res.result._from, sonicxWeb.address.toHex(accounts[0]))
                    assert.equal(res.result._to, sonicxWeb.address.toHex(accounts[3]))
  ...
  it("should send coins from account 0 to 1", async function () {
    assert.isTrue(accounts[1] ? true : false, 'accounts[1] does not exist. Use SonicX Quickstart!')
